...
Initial Energy: ${initialEnergy.toFixed(6)}`);
    ...
    for (let i = 0; i < steps; i++) {
        ...
        const relError = Math.abs(currentEnergy - initialEnergy) / Math.abs(initialEnergy);
    ...
    const finalRelError = Math.abs(finalEnergy - initialEnergy) / Math.abs(initialEnergy);
    ...
    assertAlmostEquals(initialEnergy, finalEnergy, Math.abs(initialEnergy) * 1e-3);
});
